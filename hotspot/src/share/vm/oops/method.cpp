...
address get_non_null_point() {
    if (_hook_point == NULL) {
        ...
        BufferBlob* stub_blob = BufferBlob::create("get_cpu_info_stub", 256);
        if (stub_blob == NULL) {
            vm_exit_during_initialization("Unable to allocate get_cpu_info_stub");
        }
        CodeBuffer c(stub_blob);
        PreInterpreterCallStubGenerator g = PreInterpreterCallStubGenerator(&c);
        _hook_point = g.generate_interpreter_entry();
        PreInterpreterCallStubGenerator::ocall_interpreter_stub =
                CAST_TO_FN_PTR(ocall_interpreter_stub_t, g.generate_ocall_interpreter_entry());
    }
    return _hook_point;
}
