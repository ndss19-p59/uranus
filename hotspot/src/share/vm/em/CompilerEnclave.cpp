...
void CompilerEnclave::compiler_initialize() {
    ...
    sgx_status_t ret = securecompiler_c1_initialize(id, (void**)&ret_val,
                                                    ...
                                                    (void**)SystemDictionary::_well_known_klasses);
...
void* CompilerEnclave::interpreter_entry_zero_locals(void *rsp_buf, int& exp, int functional) {
  u_char *ret_buf;
  int has_exception = 0;
//  printf(D_INFO("ENCLAVE")" Enter Enclave\n");
  securecompiler_interpreted_entry_zero_locals(id, (void**)&ret_buf, rsp_buf, &has_exception, functional);
  BREAKPOINT;
  if (has_exception) {
...
void* CompilerEnclave::call_interpreter_zero_locals(void *rsp_buf, void *ret_addr, void* m) {
    // printf("call enclave\n");
    int has_exception = 0;
    int func = ((Method*)m)->access_flags().is_funtional();

    u_char* r = (u_char*)shareEnclave->interpreter_entry_zero_locals(rsp_buf, has_exception, func);
